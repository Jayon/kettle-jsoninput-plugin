...
public class RRJsonInput extends BaseStep implements StepInterface {
	private static Class<?> PKG = RRJsonInputMeta.class; // for i18n purposes,
															// needed by
															// Translator2!!
															// $NON-NLS-1$

	private RRJsonInputMeta meta;
	private RRJsonInputData data;

	// portal
	public RRJsonInput(StepMeta stepMeta, StepDataInterface stepDataInterface,
	...
	private void handleMissingFiles() throws KettleException {
		...
		if (nonExistantFiles.size() != 0) {
			...
			log.logError(BaseMessages.getString(PKG,
					"JsonInput.Log.RequiredFilesTitle"), BaseMessages
					.getString(PKG, "JsonInput.Log.RequiredFiles", message));

			throw new KettleException(BaseMessages.getString(PKG,
					"JsonInput.Log.RequiredFilesMissing", message));
		}
		...
		if (nonAccessibleFiles.size() != 0) {
			...
			log.logError(BaseMessages.getString(PKG,
					"JsonInput.Log.RequiredFilesTitle"), BaseMessages
					.getString(PKG, "JsonInput.Log.RequiredNotAccessibleFiles",
							message));

			throw new KettleException(BaseMessages.getString(PKG,
					"JsonInput.Log.RequiredNotAccessibleFilesMissing", message));
		}
	}
	...
	private boolean ReadNextString() {
		...
		try {
			...
			if (first) {
				...
				// For String to <type> conversions, we allocate a conversion
				// meta data row as well...
				//
				data.convertRowMeta = data.outputRowMeta.clone();
				for (int i = 0; i < data.convertRowMeta.size(); i++) {
					data.convertRowMeta.getValueMeta(i).setType(
				...
				if (data.indexSourceField < 0) {
					...
					if (data.indexSourceField < 0) {
						...
						logError(BaseMessages
								.getString(
										PKG,
										"JsonInput.Log.ErrorFindingField", meta.getFieldValue())); //$NON-NLS-1$ //$NON-NLS-2$
						throw new KettleException(
								BaseMessages
										.getString(
												PKG,
												"JsonInput.Exception.CouldnotFindField", meta.getFieldValue())); //$NON-NLS-1$ //$NON-NLS-2$
					}
				}
			...
			if (log.isDetailed())
				logDetailed(BaseMessages.getString(PKG,
						"JsonInput.Log.SourceValue", meta.getFieldValue(),
						fieldValue));
			...
			if (meta.getIsAFile()) {
				...
				if (meta.isIgnoreEmptyFile()
					...
					logBasic(BaseMessages
							.getString(PKG, "JsonInput.Error.FileSizeZero",
									data.file.getName()));
					ReadNextString();
				}
			} else {
				...
				// logBasic("fieldValue"+fieldValue);
			}

			readFileOrString();
		} catch (Exception e) {
			logError(BaseMessages.getString(PKG,
					"JsonInput.Log.UnexpectedError", e.toString()));
			stopAll();
	...
	private boolean openNextFile() {
		try {
			...
			logError(BaseMessages.getString(PKG,
					"JsonInput.Log.UnableToOpenFile", "" + data.filenr,
	...
	private void readFileOrString() throws Exception {
		if (data.file != null) {
			...
			if (log.isDetailed())
				logDetailed(BaseMessages.getString(PKG,
						"JsonInput.Log.OpeningFile", data.file.toString()));
	...
	private void parseJson() throws Exception {
		...
		for (int i = 0; i < data.nrInputFields; i++) {
			...
			List<String> ja = data.jsonReader.getPath(path);
			if (data.nrrecords != -1 && data.nrrecords != ja.size()
					&& !ja.isEmpty()) {
				throw new KettleException(BaseMessages.getString(PKG,
						"JsonInput.Error.BadStructure", ja.size(), path,
						prevPath, data.nrrecords));
			}
			resultList.add(ja);
			if (data.nrrecords == -1 && !ja.isEmpty()) {
				data.nrrecords = ja.size();
			}
			prevPath = path;
		}

		data.resultList = new ArrayList<List<String>>();
		Iterator<List<String>> it = resultList.iterator();

		while (it.hasNext()) {
			List<String> j = it.next();
			if (j.isEmpty()) {
				if (data.nrrecords == -1) {
					data.nrrecords = 1;
				}
		...
		if (log.isDetailed()) {
			logDetailed(BaseMessages.getString(PKG, "JsonInput.Log.NrRecords",
					data.nrrecords));
		}
		...
		try {
			...
			if (log.isRowLevel())
				logRowlevel(BaseMessages.getString(PKG,
						"JsonInput.Log.ReadRow", data.outputRowMeta
								.getString(r)));
			incrementLinesInput();
			data.rownr++;
			...
			if (getStepMeta().isDoingErrorHandling()) {
				...
				logError(BaseMessages.getString(PKG,
						"JsonInput.ErrorInStepRunning", e.getMessage())); //$NON-NLS-1$
				setErrors(1);
				stopAll();
				setOutputDone(); // signal end to receiver(s)
				return false;
	...
	private Object[] buildRow() throws KettleException {
		...
		for (int i = 0; i < data.nrInputFields; i++) {
			// Get field
			RRJsonInputField field = meta.getInputFields()[i];

			// get json array for field
			// JSONArray
			// jsona=JSONArray.parseArray(data.resultList.get(i).toString());
			List<String> jsona = data.resultList.get(i);
			String nodevalue = null;
			if (jsona != null) {
				Object jo = (Object) jsona.get(data.recordnr);
				if (jo != null) {
					nodevalue = jo.toString().replace("\"", "");
				}
			}

			// Do trimming
			switch (field.getTrimType()) {
			case RRJsonInputField.TYPE_TRIM_LEFT:
				nodevalue = Const.ltrim(nodevalue);
				break;
			case RRJsonInputField.TYPE_TRIM_RIGHT:
				nodevalue = Const.rtrim(nodevalue);
				break;
			case RRJsonInputField.TYPE_TRIM_BOTH:
	...
	public boolean init(StepMetaInterface smi, StepDataInterface sdi) {
		meta = (RRJsonInputMeta) smi;
		data = (RRJsonInputData) sdi;
	...
	// data.file.close(),data.resultList=null
