...
/**   
 * @Title: rrjsoninput
 * @Package org.czq.rrjsoninput
 * @author eeokee@gmail.com
 * @date 2012-12-23
 * @version V1.0   
 */
 ...
 */
public class RRJsonInputMeta extends BaseStepMeta implements StepMetaInterface
{	
	private static Class<?> PKG = RRJsonInputMeta.class; // for i18n purposes, needed by Translator2!!   $NON-NLS-1$
	...
	private RRJsonInputField inputFields[];
     ...
     */
    public RRJsonInputField[] getInputFields()
    {
        return inputFields;
    }
     ...
     */
    public void setInputFields(RRJsonInputField[] inputFields)
    {
	...
	public Object clone()
	{
		RRJsonInputMeta retval = (RRJsonInputMeta)super.clone();
		...
		for (int i=0;i<nrFields;i++)
		{
            ...
            {
                retval.inputFields[i] = (RRJsonInputField)inputFields[i].clone();
            }
		}		
		return retval;
	}
    ...
    public String getXML()
    {
        ...
        {
            RRJsonInputField field = inputFields[i];
	...
	private void readData(Node stepnode) throws KettleXMLException
	{
		try
		{
			...
			for (int i=0;i<nrFields;i++)
			{
				...
				RRJsonInputField field = new RRJsonInputField(fnode);
				inputFields[i] = field;
			} 
		...
		{
			throw new KettleXMLException(BaseMessages.getString(PKG, "RRJsonInputMeta.Exception.ErrorLoadingXML", e.toString()));
		}
	}
	...
	public void allocate(int nrfiles, int nrfields)
	{
		...
		inputFields  = new RRJsonInputField[nrfields];
	}
	...
	public void setDefault()
	{
		...
		for (int i=0;i<nrFields;i++)
		{
		    inputFields[i] = new RRJsonInputField("field"+(i+1));
		}
	...
	throws KettleException
	{
		...
		try
		{
			...
			for (int i=0;i<nrFields;i++)
			{
			    RRJsonInputField field = new RRJsonInputField();
				...
				field.setTrimType( RRJsonInputField.getTrimTypeByCode( rep.getStepAttributeString (id_step, i, "field_trim_type") ));
		...
		{
			throw new KettleException(BaseMessages.getString(PKG, "RRJsonInputMeta.Exception.ErrorReadingRepository"), e);
		}
	}
	...
	throws KettleException
	{
		...
		{
			throw new KettleException(BaseMessages.getString(PKG, "RRJsonInputMeta.Exception.ErrorSavingToRepository", ""+id_step), e);
		}
	}
	...
	public void check(List<CheckResultInterface> remarks, TransMeta transMeta, StepMeta stepMeta, RowMetaInterface prev, String input[], String output[], RowMetaInterface info)
	{
		...
		if(!isInFields())
		{
			...
			if (input.length<=0)
			{		
				cr = new CheckResult(CheckResult.TYPE_RESULT_ERROR, BaseMessages.getString(PKG, "RRJsonInputMeta.CheckResult.NoInputExpected"), stepMeta);
				remarks.add(cr);
			}
			else
			{
				cr = new CheckResult(CheckResult.TYPE_RESULT_OK, BaseMessages.getString(PKG, "RRJsonInputMeta.CheckResult.NoInput"), stepMeta);
				remarks.add(cr);
			}
		}
		...
		{
			cr = new CheckResult(CheckResult.TYPE_RESULT_ERROR, BaseMessages.getString(PKG, "RRJsonInputMeta.CheckResult.NoInputField"), stepMeta);
			remarks.add(cr);
		}		
		...
		if(isInFields())
		{
			 ...
			 {
				 cr = new CheckResult(CheckResult.TYPE_RESULT_ERROR, BaseMessages.getString(PKG, "RRJsonInputMeta.CheckResult.NoField"), stepMeta);
				 remarks.add(cr); 
			 }
			 else
			 {
				 cr = new CheckResult(CheckResult.TYPE_RESULT_OK, BaseMessages.getString(PKG, "RRJsonInputMeta.CheckResult.FieldOk"), stepMeta);
				 remarks.add(cr); 
			 }		 
		}
		else
		{
			...
			{
				cr = new CheckResult(CheckResult.TYPE_RESULT_ERROR, BaseMessages.getString(PKG, "RRJsonInputMeta.CheckResult.NoFiles"), stepMeta);
				remarks.add(cr);
			}
			else
			{
				cr = new CheckResult(CheckResult.TYPE_RESULT_OK, BaseMessages.getString(PKG, "RRJsonInputMeta.CheckResult.FilesOk", ""+fileInputList.getFiles().size()), stepMeta);
				remarks.add(cr);
			}	
		}	
	}
	...
	{
		return new RRJsonInput(stepMeta, stepDataInterface, cnr, tr, trans);
	}

	public StepDataInterface getStepData()
	{
		return new RRJsonInputData();
	}
